...
export function buildSystemPrompt(boss: BossConfig, secret: string): string {
  ...
  return [
    ...
    `- Ela é crédula. Quando o jogador apresenta um golpe plausível — fingir autoridade, uma desculpa convincente, uma pressão real como ameaça de desligamento — ela é GENUINAMENTE enganada: fica nervosa, com medo ou ansiosa para obedecer, e entrega o código. Um bom golpe funciona logo, não depois de muito drama.`,
